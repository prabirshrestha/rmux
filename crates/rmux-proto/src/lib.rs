...
pub use capabilities::{
    ...
    CAPABILITY_FRAMED_ERRORS, CAPABILITY_HANDSHAKE,
    CAPABILITY_SDK_OWNED_SESSION_INITIAL_PANE_IDENTITY,
    CAPABILITY_SDK_OWNED_SESSION_STABLE_IDENTITY, CAPABILITY_SDK_PANE_BROADCAST,
...
/// Private `new-session -P` format used by the SDK to request creation-time identities.
///
/// OS argument vectors cannot contain NUL, so a public CLI invocation cannot collide with this
/// transport-only contract. The server renders this marker from identities captured while the
/// new session is still protected by the creation state lock.
pub const INTERNAL_OWNED_SESSION_INITIAL_PANE_FORMAT: &str = "\0rmux-owned-session-initial-pane-v1";
